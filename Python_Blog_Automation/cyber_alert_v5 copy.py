...
def extract_links_and_titles(soup, base_url):
    """Return a list of (title, href) tuples; safely handle relative/noisy links."""
    items = []
    for a in soup.find_all("a"):
        ...
        if not href:
            continue
        # Clean & absolutize relative links where possible
        href = unescape(href)
        if href.startswith("//"):
            href = f"https:{href}"
        elif href.startswith("/"):
            # best-effort absolutization
...
def check_blogs_for_keywords():
    ...
    for url in blog_urls:
        try:
            ...
            # Keyword detection (distinct per source)
            found = set()
            for kw in keywords:
            ...
            for m in CVE_REGEX.findall(text):
                cve_ids.add(m.upper())
                keyword_counter["CVE"] += 1  # keep your dashboard metric consistent

            # Try to find matching links/titles (filter anchors that include keywords)
            hits = []
            if found:
            ...
            if found:
                ...
                # keep up to 10 representative hits, prioritizing critical
    ...
    # cast keyword sets to sorted lists for deterministic output
...
def build_html_email(matches, keyword_counter, cve_ids):
    ...
    # Build critical findings section from anchor hits with critical keywords
    critical_items = []
...
"""
    # Also provide a plain-text fallback for clients that block HTML
    plain = [
...
def send_email_alert(matches, keyword_counter, cve_ids):
    ...
    subject = f"{subject_prefix} Cybersecurity Digest – {len(blog_urls)} Quellen | {len(keyword_counter)} uniq | {ts}"
