...
def check_blogs_for_keywords():
    """Seiten abrufen, Schlüsselwörter erkennen, CVEs extrahieren und repräsentative Artikel-Links bereitstellen."""
    ...
    for url in blog_urls:
        try:
            ...
            # Stellvertretende Links, deren Titel ein gefundenes Schlüsselwort enthalten
            hits = []
            if found:
    ...
    # Schlüsselwort-Sets abschließen (kritische zuerst)
...
def assess_severity(keyword_counter, cve_ids):
    """Gesamtbewertung und Kennzeichnung der Schwere (intern wird die englische Bezeichnung verwendet)."""
    weights = {
        "zero-day": 5,
        "ransomware": 5,
...
<!doctype html>
<html>
<head><meta charset="utf-8"><title>Blog-Sicherheitscheck: Schwachstellen im Blick</title></head>
...
"""
    ...
    for i, r in enumerate(recommendations, 1):
        plain.append(f"{i}. {r}")
    plain_text = "\n".join(plain)
    return html, plain_text, severity_en  # Die englische Schwere beibehalten für die interne Logik
