...
# -Änderungen   V3.0  Erweiterung der Stichwortliste für umfassendere Sicherheitsüberwachung
...
def build_html_email(matches, keyword_counter, cve_ids):
        ...
        for h in data.get("hits", []):
            if h["critical"]:
                ...
                critical_items.append(
                    f'<li><a href="{h["url"]}" style="text-decoration:none;color:#0a58ca;">{h["title"]}</a> {kw_badges} <span style="color:#999">({src})</span></li>'
                )
    ...
    for src, data in matches.items():
        ...
        if data["hits"]:
            items = "".join(
                f'<li><a href="{h["url"]}" style="text-decoration:none;color:#0a58ca;">{h["title"]}</a> {" ".join(badge(k) for k in h["keywords"])}</li>'
                for h in data["hits"]
            )
...
if __name__ == "__main__":
    ...
    if new_matches:
        send_email_alert(new_matches, keyword_counter, cve_ids)
    else:
        print("Keine neuen Inhalte gefunden. Keine E-Mail gesendet.")