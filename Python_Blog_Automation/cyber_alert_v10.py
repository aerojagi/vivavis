...
def fetch_url_with_retries(url, max_attempts=3, delay_seconds=2):
    """Holt eine URL mit einfacher Retry-Logik ab."""
    last_exc = None
    for attempt in range(1, max_attempts + 1):
        try:
            ...
            if attempt > 1:
                print(f"[INFO] {url} erfolgreich im Versuch {attempt} geladen.")
            return resp
        except (requests.exceptions.ConnectionError, requests.exceptions.Timeout) as e:
            last_exc = e
            if attempt < max_attempts:
                print(f"[WARN] Versuch {attempt} für {url} fehlgeschlagen: {e}. Neuer Versuch in {delay_seconds}s...")
                time.sleep(delay_seconds)
            else:
                print(f"[ERROR] {url} nach {max_attempts} Versuchen nicht erreichbar: {e}")
        except Exception as e:
            # Andere Fehler (z.B. HTTP 4xx/5xx) einmalig melden und abbrechen
            print(f"[ERROR] Fehler beim Abrufen von {url}: {e}")
            last_exc = e
            break
    raise last_exc if last_exc else RuntimeError(f"Unbekannter Fehler beim Abrufen von {url}")
...
def check_blogs_for_keywords():
    ...
    for url in blog_urls:
        try:
            ...
            # Treffer mit konkreten Link-/Titel-Zuordnungen (pro Seite dedupliziert)
            hits = []
            if found:
                hit_map = {}
                for title, href in extract_links_and_titles(soup, url):
                    tl = title.lower()
                    hit_kws = {kw for kw in found if kw.lower() in tl}
                    if not hit_kws:
                        continue

                    norm_title = title[:160]
                    key = (norm_title, href)
                    if key in hit_map:
                        # Keywords zusammenführen, Kritikalität neu bewerten
                        existing = hit_map[key]
                        merged_kws = sorted(set(existing["keywords"]).union(hit_kws))
                        existing["keywords"] = merged_kws
                        existing["critical"] = any(is_critical(k) for k in merged_kws)
                    else:
                        hit_map[key] = {
                            "title": norm_title,
                            "url": href,
                            "keywords": sorted(hit_kws),
            ...
            print(f"[WARN] Quelle nicht erreichbar oder Fehler aufgetreten: {url} – {e}")

    # Schlüsselwörter je Quelle deterministisch sortieren
...
def save_current_cves(cve_ids):
    ...
    try:
        ...
        print(f"Fehler beim Schreiben von {CVE_FILE}: {e}")
...
def build_html_email(matches, keyword_counter, cve_ids):
    ...
    # Kritische Treffer zählen: kritische Artikelhits + neue CVEs
    critical_hits_count = sum(
        1 for src, data in matches.items() for h in data.get("hits", []) if h.get("critical")
    )
    cve_count = len(cve_ids)
    critical_count = critical_hits_count + cve_count
    ...
    def badge(kw, count=None):
        ...
        return (
            ...
            f'color:#fff;border-radius:4px;padding:2px 6px;margin:2px;font-size:12px;">'
            f'{text}</span>'
        )

    top_kw_html = "".join(badge(k, c) for k, c in keyword_counter.most_common())

    # Nur NEUE CVEs anzeigen
    cve_html = ""
    if cve_ids:
        cve_html = "<p><strong>Neue CVEs erkannt:</strong> " + ", ".join(
            f'<span style="background:#8e44ad;color:#fff;border-radius:4px;'
            f'padding:2px 6px;margin:2px;">{c}</span>'
            for c in cve_ids
        ) + "</p>"

    # Kritische Meldungen aus Artikeln (dedupliziert)
    critical_items = []
    seen_critical = set()
    for src, data in matches.items():
        for h in data.get("hits", []):
            if not h.get("critical"):
                continue
            ident = (src, h["title"], h["url"])
            if ident in seen_critical:
                continue
            seen_critical.add(ident)
            kw_badges = " ".join(badge(k) for k in h["keywords"])
            critical_items.append(
                f'<li><a href="{h["url"]}">{h["title"]}</a> {kw_badges} '
    ...
    for src, data in matches.items():
        ...
        if data["hits"]:
            ...
            for h in data["hits"]:
                ident = (h["title"], h["url"])
                if ident in seen_hits:
                    continue
                seen_hits.add(ident)
                items_list.append(
                    f'<li><a href="{h["url"]}">{h["title"]}</a> '
                    f'{" ".join(badge(k) for k in h["keywords"])}</li>'
                )
            if items_list:
                links_html = f"<ul>{''.join(items_list)}</ul>"
            else:
                links_html = (
                    "<em>Keine konkreten Artikelüberschriften gefunden – "
                    "nur Begriffe im Seiteninhalt.</em>"
                )
        else:
            links_html = (
                "<em>Keine konkreten Artikelüberschriften gefunden – "
        ...
        rows.append(
            f"<tr>"
            f'<td style="vertical-align:top;padding:8px 12px;">'
            f'<a href="{src}">{src}</a></td>'
            f'<td style="vertical-align:top;padding:8px 12px;">{kw_html}</td>'
            f'<td style="vertical-align:top;padding:8px 12px;">{links_html}</td>'
            f"</tr>"
        )
...
<li>Kritische Treffer (neu): <strong style="color:#c0392b;">{critical_count}</strong></li>
</ul>
...
"""
    ...
    plain = [
        ...
        f"Quellen: {len(blog_urls)} | Gesamt: {total_hits} | "
        f"Uniq: {unique_keywords} | Kritisch (neu): {critical_count}",
        "",
        "Top-Schlüsselwörter:",
    ]
...
def build_csv_attachment(matches):
    ...
    for src, data in matches.items():
        ...
        # Hier lassen wir ggf. doppelte URLs im CSV bewusst zu – kann bei Bedarf auch dedupliziert werden
        for h in data.get("hits", []):
            kwlist = ";".join(h["keywords"])
            writer.writerow(
                [ts, src, kwlist, "yes" if h["critical"] else "no", h["title"], h["url"]]
            )
...
def send_email_alert(matches, keyword_counter, cve_ids):
    ...
    html_body, plain_body, critical_count = build_html_email(
        matches, keyword_counter, cve_ids
    )
    ts = datetime.now().strftime("%Y-%m-%d %H:%M")
    subject_prefix = f"[CRITICAL {critical_count}]" if critical_count > 0 else "[INFO]"
    subject = (
        f"{subject_prefix} Schwachstellen – {len(blog_urls)} Quellen | "
        f"{len(keyword_counter)} uniq | {ts}"
    )
...
def filter_new_matches(current_matches, previous_items):
    ...
    for src, data in current_matches.items():
        ...
        for hit in data.get("hits", []):
            identifier = f"{hit['url']}|{hit['title']}"
            if identifier in previous_items or identifier in seen_hits:
                continue
            seen_hits.add(identifier)
            new_hits.append(hit)
        if new_hits:
            new_matches[src] = {
...
def save_current_alerts(matches):
    ...
    try:
        ...
        print(f"Fehler beim Schreiben von {ALERTS_FILE}: {e}")
